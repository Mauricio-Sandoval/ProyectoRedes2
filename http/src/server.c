...
void log_event(const char *ip, const char *method, const char *path, int status) {
    ...
    if (!f) {
        printf("⚠️ No se pudo escribir en el log.\n");
...
DWORD WINAPI client_thread(LPVOID lpParam) {
    ...
    if (bytes <= 0) {
        printf("⚠️  Error recibiendo datos del cliente %s\n", ip);
        free(bundle);
        closesocket(client);
        return 0;
    }
...
int main() {
    ...
    if (setsockopt(server, SOL_SOCKET, SO_REUSEADDR, (char*)&enable, sizeof(enable)) == SOCKET_ERROR) {
        printf("⚠️  No se pudo setear SO_REUSEADDR\n");
    }
    ...
    if (bind(server, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) {
        printf("❌ bind falló (%d). Puerto en uso?\n", WSAGetLastError());
        closesocket(server);
        WSACleanup();
        getchar();
        return 1;
