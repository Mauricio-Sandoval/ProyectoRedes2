...
//  - HTTP: GET, HEAD, POST (con autenticación Basic)
//  - FTP: conectar, login, LIST, RETR (download), STOR (upload)
...
void http_menu() {
    ...
    while (1) {
        printf("\n--- HTTP Menu ---\n");
        printf("Rutas que requieren autenticacion: /status, /upload, /api/*\n");
        printf("1) GET\n2) HEAD\n3) POST\n4) Back\nOption: ");
        if (scanf("%d", &opt) != 1) { while(getchar()!='\n'); continue; }
        getchar();
        if (opt==4) break;
        printf("Path (ej: /, /status, /api/echo): ");
        fgets(path, sizeof(path), stdin); path[strcspn(path, "\n")] = 0;
        if (opt==3) {
            printf("Body: ");
            fgets(body, sizeof(body), stdin); body[strcspn(body, "\n")] = 0;
            send_http_request("POST", path, body, username, password);
        } else if (opt==1) send_http_request("GET", path, NULL, username, password);
        else if (opt==2) send_http_request("HEAD", path, NULL, username, password);
    }
}
...
void ftp_menu() {
    ...
    if (!ftp_connect(&client, FTP_SERVER_IP, FTP_SERVER_PORT)) { printf("No se pudo conectar FTP\n"); return; }
    if (!ftp_login(&client, username, password)) { printf("Login failed\n"); ftp_disconnect(&client); return; }
    ...
    while (1) {
        ...
        if (opt==1) {
            ...
            char resp[FTP_BUFFER_SIZE];
            if (!ftp_enter_passive(&client)) { printf("PASV fail\n"); continue; }
            if (!ftp_connect_data_socket(&client)) { printf("Data connect fail\n"); continue; }
            ftp_send_cmd(&client, "LIST");
            int code = ftp_recv_response(&client, resp, sizeof(resp));
            if (code != 150) { printf("LIST refused\n"); closesocket(client.data_sock); client.data_sock=INVALID_SOCKET; continue; }
            int bytes;
            while ((bytes = recv(client.data_sock, resp, sizeof(resp)-1,0))>0) { resp[bytes]=0; printf("%s", resp); }
            closesocket(client.data_sock); client.data_sock=INVALID_SOCKET;
            ftp_recv_response(&client, resp, sizeof(resp));
        } else if (opt==2) {
            printf("Remote filename to download: ");
            fgets(remote, sizeof(remote), stdin); remote[strcspn(remote, "\n")] = 0;
            printf("Save as (local): ");
            fgets(local, sizeof(local), stdin); local[strcspn(local, "\n")] = 0;
            ftp_download_file(&client, remote, local);
        } else if (opt==3) {
            printf("Local file to upload: ");
            fgets(local, sizeof(local), stdin); local[strcspn(local, "\n")] = 0;
            printf("Remote name on server: ");
            fgets(remote, sizeof(remote), stdin); remote[strcspn(remote, "\n")] = 0;
            ftp_upload_file(&client, local, remote);
        }
    }
...
int main() {
    int mainopt;
    while (1) {
        printf("\n=== Cliente combinado HTTP/FTP ===\n");
        printf("1) HTTP actions\n2) FTP actions\n3) Exit\nOption: ");
        if (scanf("%d", &mainopt) != 1) { while(getchar()!='\n'); continue; }
        getchar();
        if (mainopt == 1) http_menu();
        else if (mainopt == 2) ftp_menu();
